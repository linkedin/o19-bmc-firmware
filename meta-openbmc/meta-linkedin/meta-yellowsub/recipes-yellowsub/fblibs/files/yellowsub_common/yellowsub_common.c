...
int
yellowsub_common_fru_name(uint8_t fru, char *str) {
  ...
  switch(fru) {
    ...
    default:
#ifdef DEBUG
      syslog(LOG_WARNING, "yellowsub_common_fru_id: Wrong fru id");
#endif
    return -1;
  }
...
int
yellowsub_common_fru_id(char *str, uint8_t *fru) {
...
generate_dump(void *arg) {
  ...
  yellowsub_common_fru_name(fru, fruname);
...
int
yellowsub_common_crashdump(uint8_t fru) {
  ...
  if (t_dump[fru-1].is_running) {
    ...
    if (ret == ESRCH) {
      syslog(LOG_INFO, "yellowsub_common_crashdump: No Crashdump pthread exists");
    } else {
      ...
      system(cmd);
#ifdef DEBUG
      syslog(LOG_INFO, "yellowsub_common_crashdump: Previous crashdump thread is cancelled");
#endif
    }
  }
