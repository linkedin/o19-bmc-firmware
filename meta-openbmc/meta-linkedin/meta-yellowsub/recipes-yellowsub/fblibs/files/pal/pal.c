...
#define YELLOWSUB_PLATFORM_NAME "Yellowsub"
#define LAST_KEY "last_key"
#define YELLOWSUB_MAX_NUM_SLOTS 4
...
#define SYSCPLD_DIR "/sys/class/i2c-adapter/i2c-12/12-0031/"

#define PWM_DIR SYSCPLD_DIR
...
#define SOC_I2C_BUS 9
#define SOC_CPLD_MUX_ADD  0x77
#define SOC_PWR_BTN_LOW 10
#define SOC_PWR_BTN_HIGH 11
...
const char pal_fru_list[] = "all, slot1, slot2, slot3, slot4, mb, nic";
const char pal_fru_list_rw[] = "all, slot1, slot2, slot3, slot4, mb, nic";
const char pal_fru_list_print[] = "all, slot1, slot2, slot3, slot4";
const char pal_server_list[] = "slot1, slot2, slot3, slot4";
const char pal_fru_list_sensor_history[] = "all, slot1, slot2, slot3, slot4, mb, nic";
const int soc_channel[] = {1, 2, 4, 8};
const int nvme_channel[] = {0, 1};

size_t pal_pwm_cnt = 4;
size_t pal_tach_cnt = 8;
const char pal_pwm_list[] = "0, 1, 2, 3";
const char pal_tach_list[] = "0, 0, 1, 1, 2, 2, 3, 3";

const static uint8_t i2c_cpld_addr = 0x31;
const static uint8_t i2c_vmon_addr = 0x35;
const static uint8_t i2c_pwr_btn_addr = 0x30;
const static uint8_t i2c_sys_reset_addr = 0x31;
...
/*
 * Fan duty cycle % to fan speed control register value
 */
uint8_t pal_pwm2reg[] = {
...
int
pal_read_soccpld(uint8_t i2c_bus, uint8_t i2c_addr, uint8_t reg_offset, uint8_t *value) {
  int dev, ret, res;
  int led_offset;
  uint8_t data;
  ...
  ret = ioctl(dev, I2C_SLAVE, i2c_addr); 
  if(ret < 0) {
  ...
  res = i2c_smbus_read_byte_data(dev, reg_offset);
  if(res < 0) {
    close(dev);
    return -1;
  }
...
int
pal_write_soccpld(uint8_t i2c_bus, uint8_t i2c_addr, uint8_t reg_offset, uint8_t data) {
  int dev, ret, res;
  int led_offset;
  char bus[LARGEST_DEVICE_NAME] = "";
  ...
  res = i2c_smbus_write_byte_data(dev, reg_offset, data);
  if(res < 0) {
    close(dev);
...
server_power_on(uint8_t slot_id) {
  uint8_t i2c_bus, i2c_addr, mux_addr;
  uint8_t channel, reg, value;
  ...
  i2c_bus = SOC_I2C_BUS;
  i2c_addr = i2c_cpld_addr;
  reg = i2c_pwr_btn_addr;
  mux_addr = SOC_CPLD_MUX_ADD;
  channel = soc_channel[slot_id - 1];

  /*
   * select I2C mux channel
   */
  yellowsub_mux_sel_channel(i2c_bus, mux_addr, channel);

  /*
   * making power button high
   */
  pal_write_soccpld(i2c_bus, i2c_addr, reg, SOC_PWR_BTN_HIGH);
  sleep(DELAY_POWER_ON);

  /*
   * making power button low
   */
  pal_write_soccpld(i2c_bus, i2c_addr, reg, SOC_PWR_BTN_LOW);
  sleep(DELAY_POWER_ON);
...
pal_get_platform_name(char *name) {
  strcpy(name, YELLOWSUB_PLATFORM_NAME);
...
int
pal_hotswap_cycle(void) {
  syslog(LOG_CRIT, "HOTSWAP CYCLE");
  // Remove the adm1275 module as the HSC device is busy
  system("rmmod adm1275");

  // Send command to HSC power cycle
  system("i2cset -y 11 0x10 0xd9 c");
...
pal_set_rst_btn(uint8_t slot, uint8_t status) {
  ...
  i2c_bus = SOC_I2C_BUS;
  i2c_addr = i2c_cpld_addr;
  reg = i2c_sys_reset_addr;
  mux_addr = SOC_CPLD_MUX_ADD;
  channel = soc_channel[slot - 1];
  ...
  /*
   * set system CPLD system reset control register
   */
  if (pal_write_soccpld(i2c_bus, i2c_addr, reg, value)) {
     return -1;
  }
...
pal_get_fru_id(char *str, uint8_t *fru) {
  ...
  return yellowsub_common_fru_id(str, fru);
}

int
pal_get_fru_name(uint8_t fru, char *name) {

  return yellowsub_common_fru_name(fru, name);
}

int
pal_get_fru_sdr_path(uint8_t fru, char *path) {
  return yellowsub_sensor_sdr_path(fru, path);
}
...
pal_sensor_sdr_init(uint8_t fru, sensor_info_t *sinfo) {
  ...
  if (status)
    return yellowsub_sensor_sdr_init(fru, sinfo);
  else
    return -1;
}
...
pal_sensor_read_raw(uint8_t fru, uint8_t sensor_num, void *value) {
  ...
  while (retry) {
    ret = yellowsub_sensor_read(fru, sensor_num, value);
    if(ret >= 0)
      break;
    msleep(50);
    retry--;
  ...
  if(ret < 0) {
    ...
    if(fru == FRU_MB && sensor_num == MB_SENSOR_HSC_IN_POWER) {
...
pal_get_sensor_threshold(uint8_t fru, uint8_t sensor_num, uint8_t thresh, void *value) {
  return yellowsub_sensor_threshold(fru, sensor_num, thresh, value);
}

int
pal_get_sensor_name(uint8_t fru, uint8_t sensor_num, char *name) {
  return yellowsub_sensor_name(fru, sensor_num, name);
}

int
pal_get_sensor_units(uint8_t fru, uint8_t sensor_num, char *units) {
  return yellowsub_sensor_units(fru, sensor_num, units);
}

int
pal_get_fruid_path(uint8_t fru, char *path) {
  return yellowsub_get_fruid_path(fru, path);
}

int
pal_get_fruid_eeprom_path(uint8_t fru, char *path) {
  return yellowsub_get_fruid_eeprom_path(fru, path);
}

int
pal_get_fruid_name(uint8_t fru, char *name) {
  return yellowsub_get_fruid_name(fru, name);
}
...
pal_store_crashdump(uint8_t fru) {
  ...
  return yellowsub_common_crashdump(fru);
}
...
pal_sel_handler(uint8_t fru, uint8_t snr_num, uint8_t *event_data) {
  ...
  static int assert_cnt[YELLOWSUB_MAX_NUM_SLOTS] = {0};
...
pal_set_fan_speed(uint8_t fan, uint8_t pwm) {
  ...
  if(fan < 0 || fan >= pal_pwm_cnt) {
    syslog(LOG_INFO, "pal_set_fan_speed: fan number is invalid - %d", fan);
    return -1;
  }

  if (pwm < 0 || pwm > 100) {
     syslog(LOG_INFO, "pal_set_fan_speed: pwm is invalid - %d", pwm);
     return -1;
  }

  /*
   * 5 bits PWM speed register, about 3.125% step
   */
  val = pal_pwm2reg[pwm];

  /*
   * set system CPLD Fan PWM control register for duty cycle
   */
  ret = write_fan_value(fan+1, "fan%d_pwm_contrl", val);
  if (ret < 0) {
...
pal_get_fan_speed(uint8_t fan, int *rpm) {
   ...
   if (fan >= pal_pwm_cnt) {
       ret = sensor_cache_read(FRU_MB, MB_SENSOR_FAN0_1_TACH + fan - pal_pwm_cnt, &value);
   }
   else {
       ret = sensor_cache_read(FRU_MB, MB_SENSOR_FAN0_TACH + fan, &value);
   }
...
pal_log_clear(char *fru) {
  ...
  } else if (!strcmp(fru, "mb")) {
    pal_set_key_value("mb_sensor_health", "1");
...
pal_get_pwm_value(uint8_t fan_num, uint8_t *value) {
  char path[LARGEST_DEVICE_NAME] = {0};
  char device_name[LARGEST_DEVICE_NAME] = {0};
  float fan_rpm, percent=0;

  if(fan_num < 0 || fan_num >= pal_tach_cnt) {
    syslog(LOG_INFO, "pal_get_pwm_value: fan number is invalid - %d", fan_num);
    return -1;
  }

  /*
   * Read system CPLD Fan PWM detect register for fan speed
   */
  if (fan_num < pal_pwm_cnt) {
      /*
       * for front fan speed
       */
      read_fan_value(fan_num+1, FAN_TACH_RPM, &fan_rpm);
  }
  else {
      /*
       * for rear fan speed
       */
      read_fan_value((fan_num - pal_pwm_cnt) + 1, FAN_REAR_TACH_RPM, &fan_rpm);
  }

  /*
   * convert it to duty cycle %
   */
  if (fan_rpm < FAN_MIN_RPM)
      percent = 0.0;
  else
      percent = ((float)fan_rpm * 100.0)/((float)FAN_MAX_RPM);

  *value = (uint8_t) percent;
  return 0;
}
...
void
pal_save_boot_option(uint8_t slot_id, unsigned char* buff)
{
  int fp = 0;
  char bootoption_temp_path[64] = {0};

  sprintf(bootoption_temp_path, "/tmp/boot_slot%d.bin", slot_id);

  fp = open(bootoption_temp_path, O_WRONLY|O_CREAT);
  if(fp > 0 )
  {
    write(fp,buff,256);
    close(fp);
...
int
pal_load_boot_option(uint8_t slot_id, unsigned char* buff)
{
  int fp = 0;
  char bootoption_temp_path[64] = {0};
  ...
  fp = open(bootoption_temp_path, O_RDONLY);
  if(fp > 0 )
  {
    read(fp,buff,256);
    close(fp);
...
void
pal_set_boot_option(uint8_t fru, unsigned char para,unsigned char* pbuff)
{
  unsigned char buff[256] = { 0 };
  unsigned char offset = option_offset[para];
  unsigned char size   = option_size[para];

  pal_load_boot_option(fru, buff);
  memcpy(buff + offset, pbuff, size);
  pal_save_boot_option(fru, buff);
}

int
pal_get_boot_option(uint8_t fru, unsigned char para,unsigned char* pbuff)
{
  unsigned char buff[256] = { 0 };
  int ret = 0;
  unsigned char offset = option_offset[para];
  unsigned char size   = option_size[para];
  ret = pal_load_boot_option(fru, buff);
  if (!ret){
