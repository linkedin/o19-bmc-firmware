    ...
    if [ "$1" == "all" -o "$1" == "ALL" ]; then
        ...
        for i in $(seq 1 $END)
            do
               ...
               echo 1 > $dir/control/shutdown
               if [ ! -f $file ]; then
                   echo "eFuse not found"
                   status=1
                   continue
               fi
               #check if eFuse turned off
               output=$(cat $file)
               state="${output:0:1}"
               if [ "$state" == "1" ]; then
                   echo "Success: Turning off eFuse $i"
               else
                   echo "Failed: Turning off eFuse $i"
                   status=1
               fi
            done

        if [ $status == 0 ]; then
            echo "Success to turn all eFuses off"
        else
            echo "Failed to turn all eFuses off"
        fi
        exit 0
    fi
