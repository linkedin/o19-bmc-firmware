...
CURL_CMD = 'curl -k -H "Authorization:Basic {}" https://{}:8443{} | python -m json.tool'
CURL_CMD6 = 'curl -k -H "Authorization:Basic {}" https://[{}]:8443{} | python -m json.tool'
...
def restapiTest(data, host, logger, auth):
    curl_cmd = ""
    try:
        if check_ip6(host):
            curl_cmd = CURL_CMD6
    ...
    for endpoint in data['endpoints']:
        cmd = curl_cmd.format(auth, host, endpoint)
        ...
        try:
            ...
            for item in data['endpoints'][endpoint]:
                ...
                if item not in result:
                    ...
                    sys.exit(1)
            print(" Rest-api endpoint: " + endpoint + " test [PASSED]")
        except Exception:
            print("Rest-api test [FAILED]")
            sys.exit(1)
    print("Rest-api all test [PASSED]")
    sys.exit(0)
...
if __name__ == "__main__":
    ...
    try:
        data = {}
        args = util.Argparser(['bmc_ip', 'json', 'auth', '--verbose'],
                              [str, str, str, None],
        ...
        auth = args.auth
        restapiTest(data, host, logger, auth)
