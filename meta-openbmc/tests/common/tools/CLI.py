...
def restapiTest(hostnameBMC, data, auth, headnodeName=None):
    json = data["restapiTest.py"][1]['json']
    if HEADNODE:
        pythonfile = data["restapiTest.py"][1]['testfile']
        cmd = "python3 {0} {1} {2} {3}"
        cmd = cmd.format(pythonfile, headnodeName, json, auth)
    else:
        cmd = "python3 ../restapiTest.py {0} {1} {2}"
        cmd = cmd.format(str(hostnameBMC), str(json), str(auth))
    if VERBOSE:
        cmd = setVerbose(cmd)
    f = subprocess.Popen(
        cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    info, err = f.communicate()
    print("REST API test error: " + err)
    print(info.decode().rstrip())
    return
...
if __name__ == "__main__":
    ...
    try:
        args = util.Argparser(['json', '--headnode', 'hostnameBMC', 'hostnameMS', 'auth', '--verbose'],
                              [str, None, str, str, str, None],
                              ['a json file', 'a host name for the headnode', 'a hostname for BMC',
                              'a hostname for the Micro server', 'REST API authentication',
        ...
        if "restapiTest.py" in data:
            if HEADNODE:
                if data["restapiTest.py"][0] == 'yes':
                    restapiTest(hostnameBMC, data, headnodeName, auth)
            else:
                if data["restapiTest.py"][0] == 'yes':
                    restapiTest(hostnameBMC, data, auth)
