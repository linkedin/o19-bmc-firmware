...
void __attribute__((weak))
pal_set_boot_option(uint8_t slot, unsigned char para,unsigned char* pbuff)
{
  return;
}

int __attribute__((weak))
pal_get_boot_option(uint8_t slot, unsigned char para,unsigned char* pbuff)
{
  return PAL_ENOTSUP;
}
...
int __attribute__((weak))
function_conversation(int num_msg, const struct pam_message **msg, struct pam_response **resp, void *appdata_ptr)
{
    *resp = &reply;
    return PAM_SUCCESS;
}
 ...
 */
int __attribute__((weak))
pal_authenticate_user(char *username, char *password)
{
    ...
    if (retval != PAM_SUCCESS)
    {
        if (retval == PAM_AUTH_ERR) {
            ...
            syslog(LOG_WARNING, "pam_authenticate returned %d\n", retval);
        }
        return -1;
    }
    else
       syslog(LOG_WARNING, "Authentication success.\n");

    retval = pam_end(local_auth_handle, retval);

    if (retval != PAM_SUCCESS)
    {
        syslog(LOG_WARNING, "pam_end returned\n");
        return -1;
    }
