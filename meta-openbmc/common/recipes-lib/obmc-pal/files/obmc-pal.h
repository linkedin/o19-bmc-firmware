...
void pal_set_boot_option(uint8_t slot, unsigned char para,unsigned char* pbuff);
int pal_get_boot_option(uint8_t slot, unsigned char para,unsigned char* pbuff);
...
int pal_authenticate_user(char *username, char *password);
#ifdef __cplusplus
}
#endif
