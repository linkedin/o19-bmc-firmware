...
void get_fan_speed(uint8_t fan, uint8_t fan_tach)
{
    int ret;
    int rpm = 0;
    uint8_t pwm = 0;
    ...
    if (fan > pal_pwm_cnt) {
        printf("invalid fan number: %d\n", fan);
        return;
    }

    if (fan > pal_tach_cnt) {
        printf("invalid fan tach number: %d\n", fan_tach);
        return;
    }
     ...
     * Get the Fan Speed
     */
    ret = pal_get_fan_speed(fan_tach, &rpm);
    if (!ret) {
        memset(fan_name, 0, 32);
        pal_get_fan_name(fan_tach, fan_name);
        if ((pal_get_pwm_value(fan, &pwm)) == 0)
...
main(int argc, char **argv) {
  ...
  if ((!strcmp(argv[1], CMD_SET_FAN_STR)) && (argc == 3 || argc == 4)) {
    ...
    // Get Fan Number, if mentioned
    if (argc == 3) {
      if (parse_fan(argv[2], pal_pwm_cnt, &fan)) {
        print_usage();
        return -1;
      }
    }
