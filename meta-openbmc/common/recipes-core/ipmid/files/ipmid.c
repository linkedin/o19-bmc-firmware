...
#define CHASSIS_GET_BOOT_OPTION_SUPPORT 1
#define CHASSIS_SET_BOOT_OPTION_SUPPORT 1
...
extern int plat_lan_set(lan_config_t *lan, uint8_t option);
                          ...
                          unsigned char *response, unsigned char *res_len)
{
  ...
  *res_len = pal_get_boot_option(req->payload_id, param, data) + 2;
}
#endif
                          ...
                          unsigned char *response, unsigned char *res_len)
{
  ...
  pal_set_boot_option(req->payload_id, param,req->data+1);
...
// Get Channel Info (IPMI/Section 22.24)
static void
app_get_channel_info (unsigned char *request, unsigned char req_len,
                      unsigned char *response, unsigned char *res_len)
{
  ipmi_res_t *res = (ipmi_res_t *) response;
  unsigned char *data = &res->data[0];
  ...
  *data++ = CHANNEL_MEDIUM_LAN;
  *data++ = 1;   //IPMI_CHANNEL_PROTOCOL_IPMB;
  *data++ = 0;   //Session support
  *data++ = 0xF2;   //Vendor ID
  *data++ = 0x1B;   //Vendor ID
  *data++ = 0x00;   //Vendor ID
  *data++ = 0;
                ...
                unsigned char *response, unsigned char *res_len)
{
  ...
  switch (cmd)
  {
    ...
    case CMD_APP_GET_CHANNEL_INFO:
      app_get_channel_info (request,  req_len, response, res_len);
      break;
    default:
      res->cc = CC_INVALID_CMD;
      break;
        ...
        unsigned char *res_len)
{
  ...
  *data++ = 0x11;   // Parameter revision
  ...
  g_lan_config.channel = req->data[0] & 0xf;
  ...
  switch (param)
  {
    ...
    case LAN_PARAM_IP_ADDR:
      ...
      if (plat_lan_set(&g_lan_config, LAN_PARAM_IP_ADDR) < 0) {
          res->cc = CC_UNSPECIFIED_ERROR;
      }
      break;
    case LAN_PARAM_IP_SRC:
      g_lan_config.ip_src = req->data[2];
      if (plat_lan_set(&g_lan_config, LAN_PARAM_IP_SRC) < 0) {
          res->cc = CC_UNSPECIFIED_ERROR;
      }
      break;
    ...
    case LAN_PARAM_NET_MASK:
      ...
      if (plat_lan_set(&g_lan_config, LAN_PARAM_NET_MASK) < 0) {
          res->cc = CC_UNSPECIFIED_ERROR;
      }
      break;
    ...
    case LAN_PARAM_DF_GW_IP_ADDR:
      ...
      if (plat_lan_set(&g_lan_config, LAN_PARAM_DF_GW_IP_ADDR) < 0) {
          res->cc = CC_UNSPECIFIED_ERROR;
      }
      break;
    ...
    case LAN_PARAM_VLAN_ID:
      g_lan_config.vlanId = (req->data[3] & 0xf) << 8 | req->data[2];
      g_lan_config.vlan_enable = (req->data[3] & 0x80) ? 1 : 0;
      if (plat_lan_set(&g_lan_config, LAN_PARAM_VLAN_ID) < 0) {
          res->cc = CC_UNSPECIFIED_ERROR;
      }
      break;
  ...
  switch (param)
  {
   ...
   case LAN_PARAM_IP6_DYNAMIC_ADDR:
    ...
    case LAN_PARAM_VLAN_ID:
      plat_lan_init(&g_lan_config);
      /*
       * Least significant 8-bits of the VLAN ID. 00h if VLAN ID not used.
       */
      data[0] = g_lan_config.vlanId & 0x00ff;
      /*
       * [7]   - VLAN ID enable
       * [3:0] - most significant four bits of the VLAN ID
       */
      data[1] = (g_lan_config.vlanId & 0xff00) >> 8;

      if (g_lan_config.vlan_enable)
          data[1] |= 0x80;
      else
          data[1] &= 0x7F;
      ...
      data += 2;
      break;
    case LAN_PARAM_VLAN_PRIO:
      plat_lan_init(&g_lan_config);
      data[0] = g_lan_config.vlan_priority;
      data ++;
      break;
...
main (void)
{
  ...
  snprintf((char*)g_lan_config.if_name, IFNAMSIZ, "%s", ETH_INTF_NAME);
