...
#define PWR_OPTION_LIST "status, off, on, reset, cycle"
#endif
...
is_power_cmd_valid(char *option)
{
  ...
  if (!strcmp(option, "hotswap-cycle")) {
    return true;
  }
...
get_power_opt(char *option, uint8_t *opt) {
  ...
  if (!strcmp(option, "status")) {
    ...
    *opt = PWR_SLED_CYCLE;
  } else if (!strcmp(option, "hotswap-cycle")) {
    *opt = PWR_HOTSWAP_CYCLE;
  } else {
    return -1;
  }
...
power_util(uint8_t fru, uint8_t opt) {
  ...
  switch(opt) {
    ...
    case PWR_HOTSWAP_CYCLE:
      syslog(LOG_CRIT, "HOTSWAP_CYCLE starting...");
      sleep(1);
      pal_hotswap_cycle();
      break;
...
main(int argc, char **argv) {
  ...
  /* If argc is 2, the option is hotswap-cycle;  we should ignore power-util fru hotswap-cycle */
  if ((ret < 0) || ((argc == 2) && (opt != PWR_SLED_CYCLE) && (opt != PWR_HOTSWAP_CYCLE)) ||
      (argc == 3 && ((opt == PWR_SLED_CYCLE) || (opt == PWR_HOTSWAP_CYCLE)))) {
