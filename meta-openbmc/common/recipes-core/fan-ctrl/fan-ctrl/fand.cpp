...
int main(int argc, char **argv) {
  ...
  while (1) {
...
#elif defined(CONFIG_YELLOWSUB)
    intake_temp = exhaust_temp = userver_temp = BAD_TEMP;
    if (yellowsub_sensor_read(FRU_MB, MB_SENSOR_INLET_TEMP, &intake_temp) ||
        yellowsub_sensor_read(FRU_MB, MB_SENSOR_OUTLET_TEMP, &exhaust_temp))
      bad_reads++;
    ...
    for (int node = 1; node <= TOTAL_1S_SERVERS; node++) {
      float new_temp;
      if (!yellowsub_sensor_read(node, BIC_SENSOR_SOC_THERM_MARGIN,
                                &new_temp)) {
