...
# Find out which device maps to 'data1' on mtd
...
DATA_CHAR_DEV=$(cat /proc/mtd | awk '{ if ($4 == "\"data1\"") print $1 }' |
...
if [ -z "$DATA_CHAR_DEV" ]
then
  echo "No data1/dataro partition found. Not mounting anything to $MOUNT_POINT."
else
  DEVICE_ID=$(echo $DATA_CHAR_DEV | tail -c 2)
  DATA_BLOCK_DEV=${DATA_CHAR_DEV/mtd/mtdblock}

  echo "data1 partition found on $DATA_BLOCK_DEV; mounting to $MOUNT_POINT."
