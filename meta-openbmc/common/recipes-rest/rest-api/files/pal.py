...
def pal_server_action(slot_id, command):
    ...
    elif command == 'boot-pxe':
        cmd = '/usr/bin/bootoption-util '+fru+ ' pxe'
    elif command == 'boot-disk':
        cmd = '/usr/bin/bootoption-util '+fru+ ' disk'
    elif command == 'boot-bios':
        cmd = '/usr/bin/bootoption-util '+fru+ ' bios'
    else:
        return -1
...
def pal_user_auth(user, password):
    ret = lpal_hndl.pal_authenticate_user(user, password)
    return ret;

def pal_update_password(user, password):
    cmd='usermod --password $(openssl passwd -1 ' + password + ') ' + user
    ret = Popen(cmd, shell=True, stdout=PIPE).stdout.read().decode()
    return ret;
